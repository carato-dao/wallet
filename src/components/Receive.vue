<template>
  <div>
    ...
    <div v-if="showScanAuth && !showScan">
      ...
      <div v-if="!isChecking">
        <div style="padding: 0 20px">
          Scannerizza un codice QR di pagamento o una card<br /><br />
        </div>
        <b-button
          type="is-primary"
    ...
    <div v-if="!showScanAuth && guestwallet && !showPin">
      <div style="padding: 30px 0">
        Inserisci la quantità di carati da ricevere dalla card appena
        scansionata.
      </div>
      <br />
      Scrivi qui la quantità da ricevere:<br />
      <b-input
        type="number"
    ...
    >
      ...
      <img
        ...
        width="300"
      /><br />
      Si prega di inserire il <b>{{ $t("insertPinTwo") }}</b>
      della card scansionata per procedere:
...
export default {
  ...
  methods: {
    ...
    async checkScanned(string) {
      ...
      const check = await contract.methods.returnEid(public_hash).call();
      ...
      if (
        ...
        app.guesthash = string;
        app.guestaddress = address;
        let iv = string.substr(0, 8) + string.substr(-8);
        app.guestwallet = iv + "*" + check;
        ...
        if (app.amount > 0) {
          ...
          if (fixedBalance >= app.amount) {
            ...
            app.$buefy.toast.open({
              duration: 5000,
              message:
                "La card non abbastanza fondi, il massimo spendibile è " +
                fixedBalance +
                "!",
              position: "is-bottom",
              type: "is-danger",
    ...
    async unlockAndSend() {
      ...
      const mnemonic = await app.did.decrypt(app.guestwallet, app.pin);
      if (mnemonic !== false) {
        app.isPaying = true;
        try {
          ...
          const tx = await contract.methods
            .transfer(app.wallet.master, fixedAmount)
            .send({ from: app.guestaddress, gasPrice: gasPrice });
